  ...
  try {
    ...
    if (validateData.error) {
      return NextResponse.json(
        { error: validateData.error.issues[0].message, status: 400 },
        { status: 400 },
      );
    }
    ...
    if (existingUser) {
      return NextResponse.json(
        { error: "Email alreay in use", status: 400 },
        { status: 400 },
      );
    }
    ...
    if (error instanceof Error) {
      return NextResponse.json(
        { error: `Internal Server Error: ${error.message}`, status: 500 },
        { status: 500 },
      );
    } else {
      return NextResponse.json(
        { error: `An unexpected error occurred: ${error}`, status: 500 },
        { status: 500 },
      );
    }
  }
